...
Дополнительно проверяется `ACCOUNT_URL` (`/api/v2/account`): если HTTP `503`, это считается падением; любой другой HTTP-ответ считается успешной проверкой. Для него используются отдельные пороги `ACCOUNT_SUCCESS_STREAK_REQUIRED` и `ACCOUNT_FAILURE_STREAK_REQUIRED`.
