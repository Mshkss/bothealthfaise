...
Дополнительно проверяется `ACCOUNT_URL` (`/api/v2/account`): успешной проверкой считаются только `2xx` и коды неавторизованного доступа (`ACCOUNT_UNAUTHORIZED_OK_STATUSES`, по умолчанию `401,403`). Остальные `4xx/5xx` считаются состоянием "сервис личного кабинета не работает". Для эндпоинта используются отдельные пороги `ACCOUNT_SUCCESS_STREAK_REQUIRED` и `ACCOUNT_FAILURE_STREAK_REQUIRED`.
