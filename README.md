...
Серверный бот для Telegram, который проверяет сайт грантов и рассылает подписчикам уведомление `Сайт ожил`, когда `POST /api/v3/auth/sign-in` начинает отвечать кодом `2xx`.
